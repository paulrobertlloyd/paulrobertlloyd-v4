...
module.exports = async item => {
  ...
  if (in_reply_to) {
    content += `<p><small>↪︎ In reply to <a class="u-in-reply-to" href="${in_reply_to}">a post on ${hostname(in_reply_to)}</a></small></p>`;
  }
  ...
  if (type === 'photo') {
    for (const item of photo) {
      content += `<figure><img src="${absoluteUrl(item.url, app.url)}" alt="${item.alt}"></figure>`;
    }
  }
