...
import places from "../../src/content/places.json" with { type: "json" };
...
const _place = (label) => placesByName.get(label);

/**
 * Decorate trip data
 * @param {object} trip - Trip data
 * @returns {object|undefined} Decorated trip data
 */
export const getTripData = (trip) => {
  ...
  for (const [index, stop] of itinerary.entries()) {
    ...
    stop.icon = transitModes[stopCategory].icon;
    ...
    if (!nextPlace) {
      console.warn(`No place data found for ${nextStop.label}`);
      return;
    }
    ...
    const feature = {
      type: "Feature",
      properties: {
        stroke: transitModes[thisPlace.category[0]].color,
        "stroke-opacity": 0.75,
        "stroke-width": 4,
      },
      geometry: {
  ...
  const emissions = Math.ceil(distance * transitModes[category].emissions);
