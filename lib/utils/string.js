...
/**
 * Get the singular or plural form of a noun
 * @param {number} count - Count
 * @param {string} noun - Singular noun
 * @param {string} [plural] - Plural noun, if not the singular plus `s`
 * @returns {string} Noun, for example `photos`
 */
export const pluralName = (count, noun, plural = `${noun}s`) =>
  count === 1 ? noun : plural;

/**
 * Get a count with a pluralised noun
 * @param {number} count - Count
 * @param {string} noun - Singular noun
 * @param {string} [plural] - Plural noun, if not the singular plus `s`
 * @returns {string} Count and noun, for example `1 post` or `No posts`
 */
export const pluralise = (count, noun, plural = `${noun}s`) =>
  `${count === 0 ? "No" : count} ${pluralName(count, noun, plural)}`;
