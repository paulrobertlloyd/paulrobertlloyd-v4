...
module.exports = string => {
  ...
  let tokens = content.replaceAll(/<script.*?<\/script>|<!--.*?-->|<style.*?<\/style>|<.*?>/g, ' ');
  tokens = tokens.replaceAll(/(?=\S*’)([a-z’]+)/gi, '');
  tokens = tokens.replaceAll(/\.\s|,|;|‘|“|”|\?|\(|\)|\[|]|\/|-|–|§|&amp;|\n/g, ' ');
  tokens = tokens.replaceAll(/\b(the|a|an|and|am|you|i|to|if|of|off|me|my|on|in|it|is|at|as|we|do|be|has|but|was|so|no|not|or|up|for)\b/gi, ' ');
