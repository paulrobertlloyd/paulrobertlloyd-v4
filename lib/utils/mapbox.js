import process from "node:process";

const overlay = (map) => {
  // GeoJSON
  if (typeof map === "object") {
    // Remove private _height and _width values
    const { _height, _width, ...geojson } = map;

    return `geojson(${encodeURIComponent(JSON.stringify(geojson))})`;
  }

  // Pin
  return map;
};

/**
 * Get static Mapbox URL
 * @param {object|string} map - Map properties
 * @param {number} height - Map height
 * @param {number} width - Map width
 * @param {string} styleId - Style ID
 * @returns {string} Mapbox static image URL
 * @see {@link https://docs.mapbox.com/api/maps/static-images/}
 */
export const getMapboxUrl = (map, height, width, styleId = "outdoors-v10") => {
  const parameters = {
    endpoint: "styles",
    endpoint_version: "v1",
    username: "mapbox",
    style_id: styleId,
    type: "static",
    overlay: overlay(map),
    position: "auto",
    size: `${width || 1200}x${height || 480}@2x`,
  };
  const path = Object.values(parameters).join("/");
  const url = new URL(path, "https://api.mapbox.com");

  url.searchParams.append("access_token", process.env.MAPBOX_TOKEN);
  url.searchParams.append("logo", false);
  url.searchParams.append("padding", 60);

  return url.href;
};
