  ...
  try {
    ...
    const data = events.map(async (event) => {
      ...
      if (event.location) {
        ...
        const address = addressString ? addressString.split(/\u{5C}\u{2C}\s?/ug) : ""; // \,
        ...
        switch (address.length) {
          ...
          case 5: {
            ...
            item.location.postal_code = address.at(3);
            item.location.country_name = address.at(-1);
            break;
          }
      ...
      if (event.url) {
        ...
        if (uid?.hostname.includes("imdb.com")) {
          ...
          item.content = `<figure class="align-pull"><img src="${movie.Poster}" alt="Poster for ‘${item.title}’" data-responsiver="false"></figure>\n\n`;
          item.content += [
            `Director\n: ${movie.Director}`,
            `Writer\n: ${movie.Writer}`,
            `Actors\n: ${movie.Actors}`,
