...
export const markdownParser = (() => {
  ...
  // markdown-it-attribution still calls `parser.utils.assign`, a helper that
  ...
  parser.utils.assign = Object.assign;
  ...
  // Content uses relative media paths (`../media/…`) so images render when the
  // content repo is browsed on GitHub. Rewrite them once here, so everything
  // downstream — the image transform, feeds, etc. — sees one form.
  const normalizeLink = parser.normalizeLink.bind(parser);
  parser.normalizeLink = (url) =>
    normalizeLink(url.replace(/^\.\.\/media\//, "/media/"));
